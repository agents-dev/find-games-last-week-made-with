<div align="center">

# 🎮 Awesome AI-Built Games

### 334 verified games. Real source. No prompt-only filler.

[![Games](https://img.shields.io/badge/GAMES-334-7c3aed?style=for-the-badge&logo=itchdotio&logoColor=white)](#game-library) [![Repositories](https://img.shields.io/badge/REPOSITORIES-239-2563eb?style=for-the-badge&logo=github&logoColor=white)](games.json) [![Three.js](https://img.shields.io/badge/3D%20GAMES-170-111827?style=for-the-badge&logo=threedotjs&logoColor=white)](#threejs-and-webgl)

[![Stars](https://img.shields.io/github/stars/agents-dev/find-games-last-week-made-with?style=for-the-badge&logo=github&color=f59e0b)](https://github.com/agents-dev/find-games-last-week-made-with/stargazers) [![Forks](https://img.shields.io/github/forks/agents-dev/find-games-last-week-made-with?style=for-the-badge&logo=github&color=06b6d4)](https://github.com/agents-dev/find-games-last-week-made-with/forks)

> **A source-verified field guide to games built with GPT-6 Astra, Claude Opus, and Claude Fable.**<br />
> Every counted entry has a real GitHub repository and playable game code. Browser and non-browser engines are separated.

</div>

---

## 🤯 What is this?

Most AI-game lists mix finished games, visual demos, empty repositories, and prompt collections. This list checks the repository, gameplay source, model evidence, and canonical GitHub identity before counting a game.

**Yes, that is 334 games. Yes, each one links to source.**

## ⚡ Collection at a glance

| 🔎 Signal | 📊 Result |
| --- | ---: |
| 🎮 Independently counted games | **334** |
| 📦 Independent repositories | **239** |
| 🧊 Three.js, WebGL, or WebGPU games | **170** |
| 🛠️ Non-browser engine games | **34** |
| 📸 Games with verified screenshot links | **13** |
| 🧠 Games with direct prompt links | **8** |

## 🏆 Top-rated picks

> **Start here. These projects have the strongest combined evidence, scope, and source quality.**

| Game | Score | Built with |
| --- | ---: | --- |
| [**SUNBREAK — Downhill Club**](https://github.com/Imirushik/sunbreak-downhill-3D-game) | ⭐ **9.5** | GPT-6 Astra |
| [**THE LONG SILENCE**](https://github.com/achimala/TheLongSilence) | ⭐ **9.5** | Claude Opus 5, Claude Fable 5.1 |
| [**The Simpsons: Hit & Run — Browser Reconstruction**](https://github.com/Vheissu/hit-and-run-web) | ⭐ **9.5** | GPT-6 Astra |
| [**AOT FABLE 5.1**](https://github.com/Ashen-Skool/Aot-Fable-5.1) | ⭐ **9.0** | Claude Fable 5.1 |
| [**ASTRA AIR COMBAT**](https://github.com/FLYING37520/astra-air-combat) | ⭐ **9.0** | GPT-6 Astra |
| [**FACET FIGHTER**](https://github.com/GOROman/gpt-6-astra-ps1-game-benchmark) | ⭐ **9.0** | GPT-6 Astra |
| [**NEON WARDEN**](https://github.com/Nipale-ai/fable-5-1-one-prompt-game) | ⭐ **9.0** | Claude Fable 5.1 |
| [**NULLSPACE**](https://github.com/marius4lui/NULLSPACE) | ⭐ **9.0** | GPT-6 Astra |
| [**Rogue Squadron — The Battle of Yavin**](https://github.com/PhiloLabs/fable51-worlds) | ⭐ **9.0** | GPT-6 Astra |
| [**Saber / Descent**](https://github.com/Vheissu/saber-battle) | ⭐ **9.0** | GPT-6 Astra |
| [**Smash Karts Arena**](https://github.com/amsminn/gpt-6-astra-smash-karts) | ⭐ **9.0** | GPT-6 Astra |
| [**Turbo Kart Rush**](https://github.com/bridge-mind/turbo-kart-rush) | ⭐ **9.0** | Claude Fable 5.1 |
| [**VECTOR RUSH**](https://github.com/ToBeWin/vector-rush) | ⭐ **9.0** | GPT-6 Astra |
| [**Airlock Escape**](https://github.com/shironagasu-ai/claude-fable5-3D-games) | ⭐ **8.5** | Claude Fable 5 |
| [**Amp Build 1**](https://github.com/zuwasi/Baboons-Over-London) | ⭐ **8.5** | Claude Fable 5 |

## 📸 Screenshot spotlight

<div align="center">

[<img src="https://raw.githubusercontent.com/DEADover/DRIFTLANDS/main/docs/banner.jpg" alt="DRIFTLANDS screenshot" width="760" />](https://github.com/DEADover/DRIFTLANDS)

**DRIFTLANDS** — verified game source and screenshot.

</div>

## Game library

> 🕹️ **Browse all verified games by category.**

Jump to a category:

- 🛠️ [Non-Browser Engines](#nonbrowser-engines) — **34 games**
- 🗺️ [Adventure, RPG, and Exploration](#adventure-rpg-and-exploration) — **30 games**
- 💥 [Action and Shooters](#action-and-shooters) — **73 games**
- 🏎️ [Racing and Vehicles](#racing-and-vehicles) — **24 games**
- 🧩 [Puzzle, Arcade, and Platformers](#puzzle-arcade-and-platformers) — **32 games**
- 🧊 [Three.js and WebGL](#threejs-and-webgl) — **53 games**
- ♟️ [Strategy, Simulation, and Sports](#strategy-simulation-and-sports) — **15 games**
- 🎮 [Other Browser Games](#other-browser-games) — **73 games**

Each compact row shows **rating**, **model**, **technology**, **FLOPS estimate**, and any verified screenshot or prompt link. The layout wraps on narrow screens and avoids horizontal table scrolling.

## Non-Browser Engines

> 🛠️ **34 verified games. Ranked by evidence-based quality score.**

- [**AOT FABLE 5.1**](https://github.com/Ashen-Skool/Aot-Fable-5.1) — ⭐ **9.0/10** · Claude Fable 5.1 · Unity 6, C#, Universal Render Pipeline, WebGL, Native macOS · 6.50 GFLOPS · _confirmed_
- [**FACET FIGHTER**](https://github.com/GOROman/gpt-6-astra-ps1-game-benchmark) — ⭐ **9.0/10** · GPT-6 Astra · C, PSn00bSDK, PlayStation 1, GitHub Actions, DuckStation · 20 MFLOPS · _creator-reported_
- [**NULLSPACE**](https://github.com/marius4lui/NULLSPACE) — ⭐ **9.0/10** · GPT-6 Astra · Godot 4.7.2, GDScript, Blender, Forward+, Linux, Windows, Android · 2.40 GFLOPS · _creator-reported_
- [**VECTOR RUSH**](https://github.com/ToBeWin/vector-rush) — ⭐ **9.0/10** · GPT-6 Astra · Godot 4.7.2, GDScript, Blender, Native macOS, WebGL2, Browser · 2.40 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/ToBeWin/vector-rush/blob/main/docs/media/menu.png) · [📸 screenshot](https://github.com/ToBeWin/vector-rush/blob/main/docs/media/gameplay.gif) · [📸 screenshot](https://github.com/ToBeWin/vector-rush/blob/main/docs/media/harbor.png) · [📸 screenshot](https://github.com/ToBeWin/vector-rush/blob/main/docs/media/wall-ride.png) · [📸 screenshot](https://github.com/ToBeWin/vector-rush/blob/main/docs/media/twilight.png)
- [**Astra Tetris**](https://github.com/Ni-Cobra/GPT-6-Astra-Minecraft-Datapack-Tetris) — ⭐ **8.5/10** · GPT-6 Astra · Minecraft Java datapack, mcfunction, Python, Three.js renderer · 600 MFLOPS · _creator-reported_
- [**Block World CN**](https://github.com/vvvvvvvvvashhh/block-world-cn) — ⭐ **8.5/10** · GPT-6 Astra · Luanti 5.17.0, Mineclonia, Lua, Native desktop · 1.60 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/vvvvvvvvvashhh/block-world-cn/blob/main/docs/menu.png) · [📸 screenshot](https://github.com/vvvvvvvvvashhh/block-world-cn/blob/main/docs/inventory.png)
- [**Calude Kodo (狩人行動)**](https://github.com/GOROman/calude-famicom-game) — ⭐ **8.5/10** · Claude Fable 5 · 6502 Assembly, C, cc65, NES / Famicom, WASM emulator, Native console · 5 MFLOPS · _confirmed_ · [📸 screenshot](https://github.com/GOROman/calude-famicom-game/blob/main/docs/title_screen.png) · [📸 screenshot](https://github.com/GOROman/calude-famicom-game/blob/main/docs/screenshot.png) · [📸 screenshot](https://github.com/GOROman/calude-famicom-game/blob/main/docs/boss_fight.png)
- [**False Ritual**](https://github.com/lucas-wyd/False-Ritual) — ⭐ **8.5/10** · GPT-6 Astra · Godot 4.3, GDScript, Forward+, macOS, Windows · 2.10 GFLOPS · _creator-reported_
- [**Melodia Melusina**](https://github.com/fromage3900/MelodiaMelusinaV2) — ⭐ **8.5/10** · GPT-6 Astra · Unreal Engine 5.8, C++, Blueprints, Blender, Native desktop · 5.20 GFLOPS · _confirmed_
- [**SHOVE**](https://github.com/robss2020/claude-fable-5-having-fun) — ⭐ **8.5/10** · Claude Fable 5, Claude Opus 4.8 · Python, CLI · 120 MFLOPS · _creator-reported_
- [**The Free Game**](https://github.com/LucasMarquesShiva/the-free-game) — ⭐ **8.5/10** · GPT-6 Astra · Godot 4.7.2, GDScript, Web export, 3D simulation · 1.10 GFLOPS · _creator-reported_
- [**Gallicus 2.0**](https://github.com/Faratas410/Gallicus-2.0) — ⭐ **8.0/10** · GPT-6 Astra · Godot, GDScript, Forward+, Windows, Steam target · 1.80 GFLOPS · _creator-reported_
- [**GemMatch**](https://github.com/dylanmtaylor/gemmatch) — ⭐ **7.5/10** · Claude Opus 4.6 · Godot 4, GDScript · 350 MFLOPS · _creator-reported_
- [**NEON DAWN // 夜明け前線**](https://github.com/bannzai/godotpractice) — ⭐ **7.5/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 300 MFLOPS · _creator-reported_
- [**Neon Overdrive**](https://github.com/adisagar2003/neon-top-down-shooter) — ⭐ **7.5/10** · Claude Opus 5 · Godot 4.7, GDScript · 380 MFLOPS · _creator-reported_
- [**こもれび回収隊**](https://github.com/bannzai/godotpractice) — ⭐ **7.5/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 750 MFLOPS · _creator-reported_
- [**ころころ工房**](https://github.com/bannzai/godotpractice) — ⭐ **7.5/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 650 MFLOPS · _creator-reported_
- [**燈環闘技**](https://github.com/bannzai/godotpractice) — ⭐ **7.5/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 260 MFLOPS · _creator-reported_
- [**Batman 2D Game**](https://github.com/arnienemeth/PythonGameBatmanWithAI) — ⭐ **7.0/10** · Claude Opus 4.5 · Python, Pygame · 250 MFLOPS · _creator-reported_
- [**Drawdown**](https://github.com/substrateagnostic/drawdown) — ⭐ **7.0/10** · Claude Opus 4.8 · Godot, GDScript · 500 MFLOPS · _creator-reported_
- [**Gauntlet Loop × Unity CLI — 2.5D RPG**](https://github.com/az9713/gauntlet-loop-unity-cli-demo) — ⭐ **7.0/10** · Claude Opus 5 · Unity, C# · 260 MFLOPS · _directory-method_
- [**monsterquest**](https://github.com/bannzai/godotpractice) — ⭐ **7.0/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 220 MFLOPS · _creator-reported_
- [**Neon Survivor**](https://github.com/zlip0/neon-survivor) — ⭐ **7.0/10** · Claude Opus 4.6 · Godot, C# · 550 MFLOPS · _creator-reported_
- [**Steamed Hams: The Game**](https://github.com/Ran4/steamed_hams_the_game_claude_opus_4_6) — ⭐ **7.0/10** · Claude Opus 4.6 · Python, Pygame · 250 MFLOPS · _creator-reported_
- [**そらいろ便**](https://github.com/bannzai/godotpractice) — ⭐ **7.0/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 220 MFLOPS · _creator-reported_
- [**星環の決闘**](https://github.com/bannzai/godotpractice) — ⭐ **7.0/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 220 MFLOPS · _creator-reported_
- [**星環防衛線**](https://github.com/bannzai/godotpractice) — ⭐ **7.0/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 220 MFLOPS · _creator-reported_
- [**燈火の巡礼**](https://github.com/bannzai/godotpractice) — ⭐ **7.0/10** · GPT-6 Astra · Godot 4.7, GDScript, GL Compatibility, Desktop export, Web export · 220 MFLOPS · _creator-reported_
- [**Breakout Pygame**](https://github.com/drjasonharrison/breakout-video-pygame) — ⭐ **6.5/10** · Claude Opus 4.5 · Python, Pygame · 180 MFLOPS · _creator-reported_
- [**Chinese Word Hunt**](https://github.com/02dtan/ChineseWordHunt) — ⭐ **6.5/10** · Claude Opus 4.5 · Python, CLI · 120 MFLOPS · _creator-reported_
- [**Doom Furniture Refurb**](https://github.com/cbx-nz/mrcrayfishfurniturerefurb-doom) — ⭐ **6.5/10** · Claude Opus 4.6 · Java, Minecraft mod · 900 MFLOPS · _creator-reported_
- [**Tetris**](https://github.com/ChrisBayer42/tetris-Claude120226) — ⭐ **6.5/10** · Claude Opus 4.6 · Python, Pygame · 180 MFLOPS · _creator-reported_
- [**Tic Tac Toe**](https://github.com/shuyangsun/az-game-tic-tac-toe) — ⭐ **6.5/10** · Claude Opus 4.7 · C++, AlphaZero API · 80 MFLOPS · _creator-reported_
- [**Xiang Qi**](https://github.com/shuyangsun/az-game-xiang-qi) — ⭐ **6.5/10** · Claude Opus 4.7 · C++, AlphaZero API · 100 MFLOPS · _creator-reported_

[⬆️ Back to game library](#game-library)

## Adventure, RPG, and Exploration

> 🗺️ **30 verified games. Ranked by evidence-based quality score.**

- [**THORNMERE: The Founding Song**](https://github.com/dgahagan/THORNMERE) — ⭐ **8.5/10** · Claude Fable 5 · JavaScript, Canvas, Web Audio API · 1.10 GFLOPS · _creator-reported_
- [**BLOCKS**](https://github.com/Nipale-ai/blocks-openworld) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL, Blender · 1.20 GFLOPS · _confirmed_
- [**Hollow Pines**](https://github.com/karelpelcak/Hollow-Pines-Claude-Fable5-Game) — ⭐ **8.0/10** · Claude Fable 5 · Three.js, JavaScript, Web Audio API · 900 MFLOPS · _confirmed_
- [**Hollowmere**](https://github.com/SalihKoza/fable5-browser-game-experiment) — ⭐ **8.0/10** · Claude Fable 5 · Phaser, TypeScript, Vite · 220 MFLOPS · _confirmed_
- [**Trashketball — Out of Office (Max)**](https://github.com/swathidbhat/gpt6-astra-max-codex-trashketball) — ⭐ **8.0/10** · GPT-6 Astra · React, TypeScript, Three.js, WebGL, Vite · 1.80 GFLOPS · _creator-reported_
- [**Trashketball (High)**](https://github.com/swathidbhat/gpt6-astra-high-codex-trashketball) — ⭐ **8.0/10** · GPT-6 Astra · React, TypeScript, Three.js, WebGL, Vite · 1.50 GFLOPS · _creator-reported_
- [**Trashketball (Ultra)**](https://github.com/swathidbhat/gpt6-astra-ultra-codex-trashketball) — ⭐ **8.0/10** · GPT-6 Astra · React, TypeScript, Three.js, WebGL, Vite · 2.20 GFLOPS · _creator-reported_
- [**DEV QUEST**](https://github.com/agutierrezclab/dev-quest) — ⭐ **7.5/10** · Claude Fable 5.1 · Phaser, JavaScript, Canvas 2D · 260 MFLOPS · _creator-reported_
- [**Ember Tactics**](https://github.com/badnewsgoonies-dot/ember-tactics) — ⭐ **7.5/10** · Claude Opus 5 · TypeScript, Browser · 850 MFLOPS · _creator-reported_
- [**Embervale**](https://github.com/NicholasAryelFerreira/embervale) — ⭐ **7.5/10** · Claude Fable 5 · HTML5 Canvas, JavaScript · 220 MFLOPS · _creator-reported_
- [**Emberwood**](https://github.com/karamvirr/fable5-canvas-game) — ⭐ **7.5/10** · Claude Fable 5 · HTML, Canvas, JavaScript · 450 MFLOPS · _creator-reported_
- [**Fable5**](https://github.com/vivmuk/fable5) — ⭐ **7.5/10** · Claude Fable 5 · HTML, JavaScript, Web Audio API · 220 MFLOPS · _creator-reported_
- [**Fallout Ember**](https://github.com/chrisfirsttt/fallout-ember) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, JavaScript, Web Audio API · 1.10 GFLOPS · _creator-reported_
- [**FireRed RPG**](https://github.com/blackscythe123/FireRed-RPG) — ⭐ **7.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 260 MFLOPS · _creator-reported_
- [**Ink Rogue**](https://github.com/evan-thedev/ink-rogue) — ⭐ **7.5/10** · Claude Fable 5.1 · Canvas 2D, HTML, JavaScript · 220 MFLOPS · _creator-reported_
- [**KINDLE**](https://github.com/TonyDowney/kindle) — ⭐ **7.5/10** · Claude Opus 5 · WebGL2, HTML, JavaScript · 900 MFLOPS · _creator-reported_
- [**Space You Land**](https://github.com/JaronKBragg7337/SYL-Full-Game) — ⭐ **7.5/10** · Claude Fable 5 · JavaScript, WebGL · 4 GFLOPS · _creator-reported_
- [**Ashlands**](https://github.com/PeterBlenessy/ashlands) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, TypeScript, WebGL2 · 1.10 GFLOPS · _directory-method_
- [**Hollow Road**](https://github.com/afteradier/first-prompt-game) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, Web Audio API · 1.20 GFLOPS · _creator-reported_
- [**Jelajah Antariksa**](https://github.com/RafiulM/3d-game-fable5.1) — ⭐ **7.0/10** · Claude Fable 5.1 · HTML, JavaScript, Three.js · 1.80 GFLOPS · _creator-reported_
- [**Neon Precinct**](https://github.com/dylanhsieh/neon-precinct) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript, WebGL · 900 MFLOPS · _directory-method_
- [**The Rooms**](https://github.com/leeshinhyuk/the-rooms-public) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 1.50 GFLOPS · _creator-reported_
- [**World of ClaudeCraft**](https://github.com/levy-street/world-of-claudecraft) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.20 GFLOPS · _creator-reported_
- [**Badge Realms**](https://github.com/0LauraGelvez/Badge-realms) — ⭐ **6.5/10** · Claude Opus 4.1 · HTML, JavaScript · 250 MFLOPS · _creator-reported_
- [**Dragon Quest-style RPG Game 2**](https://github.com/Isusun/rpg_test_fable5) — ⭐ **6.5/10** · Claude Fable 5 · HTML, JavaScript · 500 MFLOPS · _creator-reported_
- [**Dragon Quest-style RPG Game 3**](https://github.com/Isusun/rpg_fable5_3) — ⭐ **6.5/10** · Claude Fable 5 · HTML, JavaScript · 500 MFLOPS · _creator-reported_
- [**MONQUEST**](https://github.com/Ajay9o9/MONQUEST) — ⭐ **6.5/10** · Claude Fable 5 · HTML, JavaScript · 450 MFLOPS · _creator-reported_
- [**Pilgrim's Path**](https://github.com/beneisenberg/pilgrims-path) — ⭐ **6.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas · 250 MFLOPS · _creator-reported_
- [**The Mysterious Castle**](https://github.com/mike-snodgrass/text-adventure-game) — ⭐ **6.5/10** · Claude Opus 5 · HTML, JavaScript · 120 MFLOPS · _creator-reported_
- [**Threshold**](https://github.com/joshuakoester82/treshold-fable-5-1) — ⭐ **6.5/10** · Claude Fable 5.1 · HTML, JavaScript · 300 MFLOPS · _creator-reported_

[⬆️ Back to game library](#game-library)

## Action and Shooters

> 💥 **73 verified games. Ranked by evidence-based quality score.**

- [**ASTRA AIR COMBAT**](https://github.com/FLYING37520/astra-air-combat) — ⭐ **9.0/10** · GPT-6 Astra · TypeScript, Three.js, WebGL, Vite, Web Audio API · 4.80 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/FLYING37520/astra-air-combat/blob/main/artifacts/cockpit-rain-high.jpg) · [📸 screenshot](https://github.com/FLYING37520/astra-air-combat/blob/main/artifacts/islands-clear.jpg)
- [**NEON WARDEN**](https://github.com/Nipale-ai/fable-5-1-one-prompt-game) — ⭐ **9.0/10** · Claude Fable 5.1 · Three.js, GLTFLoader, WebGL, HTML, JavaScript, Blender, Browser · 5 GFLOPS · _confirmed_ · [🔗 files](https://github.com/Nipale-ai/fable-5-1-one-prompt-game/tree/main/fable) · [🧠 prompt](https://github.com/Nipale-ai/fable-5-1-one-prompt-game/blob/main/prompt.txt)
- [**Saber / Descent**](https://github.com/Vheissu/saber-battle) — ⭐ **9.0/10** · GPT-6 Astra · TypeScript, Three.js, WebGL2, Vite, Blender, Browser · 6.10 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/Vheissu/saber-battle/blob/main/screenshots/01-saber-descent-temple.png) · [📸 screenshot](https://github.com/Vheissu/saber-battle/blob/main/screenshots/03-saber-descent-combat.png) · [📸 screenshot](https://github.com/Vheissu/saber-battle/blob/main/screenshots/02-saber-descent-armory.png) · [📸 screenshot](https://github.com/Vheissu/saber-battle/blob/main/screenshots/04-saber-descent-blade-effects.png)
- [**Amp Build 1**](https://github.com/zuwasi/Baboons-Over-London) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.20 GFLOPS · _confirmed_ · [🧠 prompt](https://github.com/zuwasi/Baboons-Over-London/blob/main/prompt.txt)
- [**Amp Build 2**](https://github.com/zuwasi/Baboons-Over-London) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.20 GFLOPS · _confirmed_ · [🧠 prompt](https://github.com/zuwasi/Baboons-Over-London/blob/main/prompt.txt)
- [**Amp Build 3**](https://github.com/zuwasi/Baboons-Over-London) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.20 GFLOPS · _confirmed_ · [🧠 prompt](https://github.com/zuwasi/Baboons-Over-London/blob/main/prompt.txt)
- [**BLACKSITE**](https://github.com/Hiraeth010/blacksite) — ⭐ **8.5/10** · Claude Fable 5.1 · Three.js, TypeScript, Vite, Rapier, WebGL, Browser · 8 GFLOPS · _confirmed_
- [**BLACKWATER — Silent Harbor**](https://github.com/Hiraeth010/blackwater) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, React, TypeScript, WebGL, Vite, Browser · 8.50 GFLOPS · _confirmed_
- [**Claude Build 1**](https://github.com/zuwasi/Baboons-Over-London) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.20 GFLOPS · _confirmed_ · [🧠 prompt](https://github.com/zuwasi/Baboons-Over-London/blob/main/prompt.txt)
- [**Claude Build 2**](https://github.com/zuwasi/Baboons-Over-London) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.20 GFLOPS · _confirmed_ · [🧠 prompt](https://github.com/zuwasi/Baboons-Over-London/blob/main/prompt.txt)
- [**Claude Build 3**](https://github.com/zuwasi/Baboons-Over-London) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.20 GFLOPS · _confirmed_ · [🧠 prompt](https://github.com/zuwasi/Baboons-Over-London/blob/main/prompt.txt)
- [**Digi Garden**](https://github.com/phenomenal-manish/gpt6-astra-vs-opus5-digimon-sim) — ⭐ **8.5/10** · GPT-6 Astra · HTML, JavaScript, Canvas 2D, Web Audio API · 280 MFLOPS · _confirmed_
- [**Drone Arena**](https://github.com/shironagasu-ai/claude-fable5-3D-games) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, cannon-es, JavaScript · 1.80 GFLOPS · _confirmed_
- [**Emberlight**](https://github.com/LeiGaoRobot/emberlight) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, JavaScript, WebGL, Blender, Web Audio API, GitHub Pages · 3.50 GFLOPS · _creator-reported_
- [**Swarm**](https://github.com/shironagasu-ai/claude-fable5-3D-games) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, cannon-es, JavaScript · 1.80 GFLOPS · _confirmed_
- [**VESPER: The Hollow Earth**](https://github.com/EiNSTeiN-/astra-tomb-raider-test) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, Vite, JavaScript, WebGL2, Web Audio API · 8 GFLOPS · _inferred_
- [**Astra — Chapter I: A delivery for Gundren**](https://github.com/LucasM25-max/dnd-astra) — ⭐ **8.0/10** · GPT-6 Astra · Three.js, TypeScript, Vite, WebGL, Web Audio API · 2.50 GFLOPS · _inferred_
- [**Awawa Quest**](https://github.com/Rebell-Leader/claude-game) — ⭐ **8.0/10** · Claude Fable 5 · JavaScript, HTML, SVG · 500 MFLOPS · _creator-reported_
- [**Cube Basher**](https://github.com/mreflow/cube-basher) — ⭐ **8.0/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 1.80 GFLOPS · _creator-reported_
- [**CUSTOMS — Escape Raid**](https://github.com/INONONO66/tarkov-customs) — ⭐ **8.0/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D · 360 MFLOPS · _confirmed_
- [**Dungeon of Opus**](https://github.com/joozio/dungeon-of-opus) — ⭐ **8.0/10** · Claude Opus 4.6 · TypeScript, Browser · 700 MFLOPS · _creator-reported_
- [**Grid Wars**](https://github.com/loosecannons/grid-wars) — ⭐ **8.0/10** · Claude Fable 5, Claude Opus 4.8 · Three.js, JavaScript, WebGL · 1.50 GFLOPS · _confirmed_
- [**HELLGRID**](https://github.com/markcastle/hellgrid) — ⭐ **8.0/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 1.80 GFLOPS · _creator-reported_
- [**Neon Arena**](https://github.com/B-Blarr/Arena-Game) — ⭐ **8.0/10** · Claude Fable 5 · TypeScript, Vite, Three.js · 1.80 GFLOPS · _creator-reported_
- [**Nightfall**](https://github.com/AlfredBrowser/Nightfall-Call-of-Duty-Remake-Fable-5.1) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL · 900 MFLOPS · _confirmed_
- [**Pinehollow Funland**](https://github.com/AlfredBrowser/Pinehollow-Funland-Five-Nights-at-Freddys-Remake-Fable-5.1) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL, Howler · 900 MFLOPS · _confirmed_
- [**Reablo 2**](https://github.com/oeo/reablo-2) — ⭐ **8.0/10** · Claude Fable 5.1 · TypeScript, Canvas 2D, Bun, WebSocket · 260 MFLOPS · _confirmed_
- [**Rocket Arena**](https://github.com/AlfredBrowser/Rocket-Arena-Rocket-League-Remake-Fable-5.1) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL · 900 MFLOPS · _confirmed_
- [**Voxelcraft**](https://github.com/AlfredBrowser/Voxelcraft-Minecraft-Remake-Fable-5.1) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL · 1.10 GFLOPS · _confirmed_
- [**A Thousand Skies**](https://github.com/threapchills/MagicCarpetWizard) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, Vite, WebGL, Web Audio API · 1.50 GFLOPS · _creator-reported_
- [**Backroom Escape**](https://github.com/StarKnightt/Backroom-Escape) — ⭐ **7.5/10** · Claude Fable 5 · Three.js, Next.js, JavaScript · 900 MFLOPS · _creator-reported_
- [**Claude of Duty**](https://github.com/brynary/claude-of-duty) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, TypeScript, Web Audio API · 1.20 GFLOPS · _creator-reported_
- [**Claude of Duty: Phadda**](https://github.com/zainzafar90/call-of-duty-phadda) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, Rapier, JavaScript, WebGL · 1.80 GFLOPS · _confirmed_
- [**Claude\Zombies**](https://github.com/justinw916-sketch/claude-zombies) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, TypeScript, WebGL2, Web Audio API · 900 MFLOPS · _creator-reported_
- [**Clawd Pop 3D**](https://github.com/VibezZzCoder/clawd-pop) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, JavaScript, Web Audio API · 1.20 GFLOPS · _creator-reported_
- [**Codex of Duty**](https://github.com/luongnv89/codex-of-duty) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, Rapier, JavaScript, WebGL2 · 1.80 GFLOPS · _creator-reported_
- [**CS Clone Fable5**](https://github.com/Gabry9110/CS-Clone-Fable5) — ⭐ **7.5/10** · Claude Fable 5 · Three.js, Node.js, JavaScript · 1.20 GFLOPS · _creator-reported_
- [**Dust Corridor**](https://github.com/Leonxlnx/claude-of-duty) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, JavaScript, WebGL2, Web Audio API · 1.20 GFLOPS · _creator-reported_
- [**Gogh Strike**](https://github.com/petergpt/gogh-strike) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, WebGL, JavaScript · 1.50 GFLOPS · _creator-reported_
- [**Malandro — Cidade Maravilhosa**](https://github.com/herval/gta3-rj-opus5) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, WebGL, JavaScript, Web Audio API · 1.20 GFLOPS · _creator-reported_
- [**Metal Assault**](https://github.com/AndreaZero/metal-assault-game-claude-fable) — ⭐ **7.5/10** · Claude Fable 5 · HTML5 Canvas, JavaScript, Web Audio API · 450 MFLOPS · _creator-reported_
- [**Nerd of Duty**](https://github.com/sytaylor/nerd-of-duty) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, JavaScript, WebGL2 · 900 MFLOPS · _creator-reported_
- [**Operation Blackout**](https://github.com/taozhuo/operation-blackout) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, WebGL2, JavaScript · 900 MFLOPS · _creator-reported_
- [**Operation Ironhold**](https://github.com/StarKnightt/operation-ironhold) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, HTML, JavaScript · 900 MFLOPS · _creator-reported_
- [**Origin 16-bit ARPG**](https://github.com/DFarm6/origin-16bit-arpg) — ⭐ **7.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 260 MFLOPS · _creator-reported_
- [**Perielio**](https://github.com/carlosresendeP/perielio-fable5-game) — ⭐ **7.5/10** · Claude Fable 5 · Canvas 2D, Web Audio API, JavaScript · 260 MFLOPS · _creator-reported_
- [**Sands of the Restless**](https://github.com/eddiebelaval/sands-of-the-restless) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, JavaScript, Web Audio API · 1.10 GFLOPS · _creator-reported_
- [**Starfall Scavenger**](https://github.com/Hanazar-Games/claude-opus5-aigc-webgame-project) — ⭐ **7.5/10** · Claude Opus 5 · Canvas 2D, JavaScript, Web Audio API · 360 MFLOPS · _creator-reported_
- [**Sunset Hollow**](https://github.com/sajalrajhans1/Sunset-Down) — ⭐ **7.5/10** · Claude Opus 5 · TypeScript, Three.js, Vite · 1.80 GFLOPS · _creator-reported_
- [**THRESHOLD**](https://github.com/CantankerousPotatomancer/threshold-fable-5-1) — ⭐ **7.5/10** · Claude Fable 5.1 · WebGL2, JavaScript, Web Audio API · 1.20 GFLOPS · _creator-reported_
- [**Whirlwind Ascension**](https://github.com/dylanhsieh/whirlwind-ascension) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, HTML, JavaScript · 1.10 GFLOPS · _creator-reported_
- [**Workmelt**](https://github.com/eriknomitch/workmelt) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, WebGL2, JavaScript, WebSockets · 1.20 GFLOPS · _creator-reported_
- [**3D Space Shooter**](https://github.com/mihaibulic2/test-game-claude-4.7) — ⭐ **7.0/10** · Claude Opus 4.7 · JavaScript, Three.js · 1.60 GFLOPS · _creator-reported_
- [**3DFPS**](https://github.com/wiswis01/3DFPS) — ⭐ **7.0/10** · Claude Opus 4.5 · HTML, JavaScript, Three.js · 1.80 GFLOPS · _creator-reported_
- [**ASHFALL**](https://github.com/alessoh/game) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, WebGL, Web Audio API · 900 MFLOPS · _directory-method_
- [**Beach Volley FPS**](https://github.com/teoobarca/beach-volley-fps) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript, WebGL · 900 MFLOPS · _directory-method_
- [**Call of Doodie**](https://github.com/VaultSparkStudios/call-of-doodie) — ⭐ **7.0/10** · Claude Opus 4.6 · JavaScript, Browser · 800 MFLOPS · _creator-reported_
- [**Coro Solto**](https://github.com/corosolto/client) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript, WebGL · 900 MFLOPS · _directory-method_
- [**Dogs of War**](https://github.com/slampunk/dogs-of-war) — ⭐ **7.0/10** · Claude Opus 5 · HTML, JavaScript · 900 MFLOPS · _creator-reported_
- [**Everdeep**](https://github.com/KeigoShimadaCC/generative-gaming) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _creator-reported_
- [**Forest Pests**](https://github.com/fjzeit/forest-pests) — ⭐ **7.0/10** · Claude Opus 4.5 · TypeScript, Browser · 350 MFLOPS · _creator-reported_
- [**Group Survival Game**](https://github.com/kaykayyali/Group-Survival-Game) — ⭐ **7.0/10** · Claude Opus 5 · Phaser, JavaScript, WebSockets · 220 MFLOPS · _directory-method_
- [**Iridium Reach**](https://github.com/jason-c-dev/iridium-reach-demo) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript, Web Audio API · 1.50 GFLOPS · _directory-method_
- [**Operation Blackout**](https://github.com/erekjiang/call-of-duty-opus5) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, WebGL, JavaScript, Web Audio API · 1.80 GFLOPS · _repository/topic trail_
- [**Pastel Nuketown**](https://github.com/luckeyfaraday/pastel-nuketown) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript, WebSockets · 1.20 GFLOPS · _creator-reported_
- [**Riftbound: Meridian Trial**](https://github.com/TheXUMU9/TheXUMU9-Game_with-Gauntlet_Loop) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript, Vite · 1.10 GFLOPS · _directory-method_
- [**SpaceHo2**](https://github.com/PierreHoule/SpaceHo2) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript · 350 MFLOPS · _creator-reported_
- [**Trench Assault 1943**](https://github.com/tenbonks/trench-assault-1943) — ⭐ **7.0/10** · Claude Fable 5, Claude Opus 4.8 · JavaScript, HTML5 Canvas · 650 MFLOPS · _creator-reported_
- [**Turbo Turf Wars**](https://github.com/shreyasr25/TurboTurfWars) — ⭐ **7.0/10** · Claude Opus 4.6 · HTML, JavaScript · 400 MFLOPS · _creator-reported_
- [**ZOF//DUTY**](https://github.com/mike007jd/ZofDuty) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, WebGL2 · 900 MFLOPS · _directory-method_
- [**Claude of Duty**](https://github.com/mshumer/Claude-of-Duty) — ⭐ **6.5/10** · Claude Opus 5 · Three.js, WebGL2 · 900 MFLOPS · _directory-method_
- [**Neon Dodge Arena**](https://github.com/Jabaraj16/neon-dodge-arena) — ⭐ **6.5/10** · Claude Opus 5 · TypeScript, Three.js · 1 GFLOPS · _creator-reported_
- [**Warehouse FPS**](https://github.com/404-Repo/404-game-recipe) — ⭐ **6.5/10** · Claude Opus 5 · Three.js, JavaScript · 1.20 GFLOPS · _directory-method_

[⬆️ Back to game library](#game-library)

## Racing and Vehicles

> 🏎️ **24 verified games. Ranked by evidence-based quality score.**

- [**THE LONG SILENCE**](https://github.com/achimala/TheLongSilence) — ⭐ **9.5/10** · Claude Opus 5, Claude Fable 5.1 · Three.js, WebGL2, GLSL, JavaScript, Vite, Browser · 15 GFLOPS · _confirmed_
- [**Rogue Squadron — The Battle of Yavin**](https://github.com/PhiloLabs/fable51-worlds) — ⭐ **9.0/10** · GPT-6 Astra · TypeScript, React, Three.js, WebGL, GLSL, Vite · 4.20 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/PhiloLabs/fable51-worlds/blob/main/death-star-trench-run-gpt-astra/media/preview.gif)
- [**Smash Karts Arena**](https://github.com/amsminn/gpt-6-astra-smash-karts) — ⭐ **9.0/10** · GPT-6 Astra · Three.js, React, TypeScript, Vite, WebSocket, Node.js, Web Audio API · 2.20 GFLOPS · _creator-reported_
- [**Turbo Kart Rush**](https://github.com/bridge-mind/turbo-kart-rush) — ⭐ **9.0/10** · Claude Fable 5.1 · Three.js, TypeScript, Vite, WebGL2, Web Audio API, Browser · 7 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/bridge-mind/turbo-kart-rush/blob/main/docs/screenshots/title.jpg) · [📸 screenshot](https://github.com/bridge-mind/turbo-kart-rush/blob/main/docs/screenshots/character-select.jpg) · [📸 screenshot](https://github.com/bridge-mind/turbo-kart-rush/blob/main/docs/screenshots/track-select.jpg) · [📸 screenshot](https://github.com/bridge-mind/turbo-kart-rush/blob/main/docs/screenshots/race.jpg)
- [**Drift Away Arrows**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Fable Kart**](https://github.com/koviq4/fable-sandbox) — ⭐ **8.5/10** · Claude Fable 5.1 · Three.js, Vite, JavaScript · 900 MFLOPS · _confirmed_
- [**Turbo Kart Grand Prix**](https://github.com/Franklin-C/turbo-kart-grand-prix) — ⭐ **8.5/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL, Web Audio API · 2.40 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/Franklin-C/turbo-kart-grand-prix/blob/main/docs/race.jpg) · [📸 screenshot](https://github.com/Franklin-C/turbo-kart-grand-prix/blob/main/docs/menu.jpg) · [📸 screenshot](https://github.com/Franklin-C/turbo-kart-grand-prix/blob/main/docs/hill.jpg) · [📸 screenshot](https://github.com/Franklin-C/turbo-kart-grand-prix/blob/main/docs/star.jpg)
- [**Frostline Kart**](https://github.com/sorrycc/fable-arcade) — ⭐ **8.2/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D, WebGL, Browser · 800 MFLOPS · _confirmed_ · [🔗 files](https://github.com/sorrycc/fable-arcade/tree/main/games/mario-kart-snow) · [▶️ play](https://sorrycc.github.io/fable-arcade/games/mario-kart-snow/)
- [**Apex Formula**](https://github.com/bridge-mind/apex-formula) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, TypeScript, WebGL · 1.80 GFLOPS · _confirmed_
- [**Clay Kart**](https://github.com/godmosword/clay-kart) — ⭐ **8.0/10** · Claude Opus 5 · JavaScript, HTML, CSS · 220 MFLOPS · _confirmed_
- [**Fable Flight**](https://github.com/Arnie016/flight-simulator-fable5) — ⭐ **8.0/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 2.20 GFLOPS · _creator-reported_
- [**Flip Runner Racing**](https://github.com/markcastle/fliprunner) — ⭐ **8.0/10** · Claude Fable 5 · HTML, JavaScript, Canvas · 350 MFLOPS · _creator-reported_
- [**Iron Man: Legion Protocol**](https://github.com/Aaryan-Kapoor/Iron-Man-Fable-5) — ⭐ **8.0/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 3.50 GFLOPS · _creator-reported_
- [**Murmur**](https://github.com/pahorema/murmur) — ⭐ **8.0/10** · Claude Opus 5 · Three.js, WebGPU, JavaScript · 2.90 GFLOPS · _confirmed_
- [**No AI's Sky**](https://github.com/jesuscurreripa/noaissky) — ⭐ **8.0/10** · GPT-6 Astra · Three.js, Vite, JavaScript, WebGL · 3 GFLOPS · _creator-reported_
- [**AEON DRIFT**](https://github.com/TommyUSN/opus5-tech-demo) — ⭐ **7.5/10** · Claude Opus 5 · WebGL2, JavaScript, Web Audio API · 900 MFLOPS · _creator-reported_
- [**DESCENT**](https://github.com/sjwebster-bne/downhillmtb) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, JavaScript, Web Audio API · 1.20 GFLOPS · _creator-reported_
- [**Sunset Flap**](https://github.com/5kahoisaac/sunset-flap) — ⭐ **7.5/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 700 MFLOPS · _creator-reported_
- [**APEX GP**](https://github.com/jolbol1/apex-gp) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript · 900 MFLOPS · _directory-method_
- [**Car Game at Sunset**](https://github.com/taarichandaria/car-game-fable-5-test) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 2.20 GFLOPS · _creator-reported_
- [**DRIFTLANDS**](https://github.com/DEADover/DRIFTLANDS) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript, Vite · 900 MFLOPS · _directory-method_ · [📸 screenshot](https://github.com/DEADover/DRIFTLANDS/blob/main/docs/banner.jpg)
- [**Kart Royale**](https://github.com/ryancampbell/kart-royale) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, TypeScript, Vite · 900 MFLOPS · _directory-method_
- [**Outrun O5**](https://github.com/yonifra/-outrun-o5) — ⭐ **7.0/10** · Claude Opus 5 · HTML, JavaScript, WebGL · 1.60 GFLOPS · _creator-reported_
- [**Vibe Pixel Rally**](https://github.com/jv4v2nw88z-hue/VibePixleRalley) — ⭐ **6.5/10** · Claude Opus 5 · JavaScript, Canvas · 350 MFLOPS · _creator-reported_

[⬆️ Back to game library](#game-library)

## Puzzle, Arcade, and Platformers

> 🧩 **32 verified games. Ranked by evidence-based quality score.**

- [**CityMaker**](https://github.com/derek-wangpch/OpenCityMaker) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, React, TypeScript, Vite, WebGL, IndexedDB · 800 MFLOPS · _creator-reported_
- [**Frost Hop**](https://github.com/MrAllNeo/frost-hop) — ⭐ **8.5/10** · GPT-6 Astra · React, JavaScript, Canvas 2D, Web Audio API, Vite · 650 MFLOPS · _confirmed_
- [**MirrorForge**](https://github.com/leonvanzyl/claude-fable-5-mirror-game) — ⭐ **8.5/10** · Claude Fable 5 · TypeScript, WebGPU, WGSL · 4.50 GFLOPS · _creator-reported_
- [**Silent Meridian**](https://github.com/stackloomdev/silent-meridian) — ⭐ **8.5/10** · GPT-6 Astra · HTML, JavaScript, WebGL, Web Audio API, Vercel · 650 MFLOPS · _creator-reported_
- [**Super Fable Bros. — World 1-1**](https://github.com/INONONO66/fable-5.1-mario) — ⭐ **8.5/10** · Claude Fable 5.1 · Canvas 2D, JavaScript, Web Audio API, Browser · 250 MFLOPS · _confirmed_ · [📸 screenshot](https://github.com/INONONO66/fable-5.1-mario/blob/main/docs/title.png) · [📸 screenshot](https://github.com/INONONO66/fable-5.1-mario/blob/main/docs/gameplay.png) · [📸 screenshot](https://github.com/INONONO66/fable-5.1-mario/blob/main/docs/gameplay.gif)
- [**Wormhole**](https://github.com/AidanHT/Wormhole) — ⭐ **8.5/10** · Claude Fable 5 · TypeScript, Vite, WebGL · 2.20 GFLOPS · _creator-reported_
- [**Fable Theft Auto 5.1**](https://github.com/sorrycc/fable-arcade) — ⭐ **8.2/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D, WebGL, Browser · 900 MFLOPS · _confirmed_ · [🔗 files](https://github.com/sorrycc/fable-arcade/tree/main/games/voxel-gta-city) · [▶️ play](https://sorrycc.github.io/fable-arcade/games/voxel-gta-city/)
- [**Hollowmere**](https://github.com/sorrycc/fable-arcade) — ⭐ **8.0/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D, WebGL, Browser · 700 MFLOPS · _confirmed_ · [🔗 files](https://github.com/sorrycc/fable-arcade/tree/main/games/terraria-sandbox) · [▶️ play](https://sorrycc.github.io/fable-arcade/games/terraria-sandbox/)
- [**Rail Rush**](https://github.com/sorrycc/fable-arcade) — ⭐ **7.8/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D, WebGL, Browser · 450 MFLOPS · _confirmed_ · [🔗 files](https://github.com/sorrycc/fable-arcade/tree/main/games/subway-runner) · [▶️ play](https://sorrycc.github.io/fable-arcade/games/subway-runner/)
- [**Robo Tennis**](https://github.com/sorrycc/fable-arcade) — ⭐ **7.8/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D, WebGL, Browser · 500 MFLOPS · _confirmed_ · [🔗 files](https://github.com/sorrycc/fable-arcade/tree/main/games/robot-tennis) · [▶️ play](https://sorrycc.github.io/fable-arcade/games/robot-tennis/)
- [**World 1-1**](https://github.com/sorrycc/fable-arcade) — ⭐ **7.8/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D, WebGL, Browser · 500 MFLOPS · _confirmed_ · [🔗 files](https://github.com/sorrycc/fable-arcade/tree/main/games/mario-1-1) · [▶️ play](https://sorrycc.github.io/fable-arcade/games/mario-1-1/)
- [**Beyond Boring: The Game**](https://github.com/kolatts/beyond-boring-the-game) — ⭐ **7.5/10** · Claude Fable 5 · Three.js, TypeScript, Vite · 1.10 GFLOPS · _creator-reported_
- [**Crossy Farm**](https://github.com/sorrycc/fable-arcade) — ⭐ **7.5/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D, WebGL, Browser · 350 MFLOPS · _confirmed_ · [🔗 files](https://github.com/sorrycc/fable-arcade/tree/main/games/crossy-farm-car) · [▶️ play](https://sorrycc.github.io/fable-arcade/games/crossy-farm-car/)
- [**Flappy Skies**](https://github.com/sorrycc/fable-arcade) — ⭐ **7.5/10** · Claude Fable 5.1 · HTML, JavaScript, Canvas 2D, WebGL, Browser · 350 MFLOPS · _confirmed_ · [🔗 files](https://github.com/sorrycc/fable-arcade/tree/main/games/flappy-bird) · [▶️ play](https://sorrycc.github.io/fable-arcade/games/flappy-bird/)
- [**Flip Match**](https://github.com/asmoyou/toy2game) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, TypeScript, Vite, cannon-es · 1.80 GFLOPS · _repository/topic trail_
- [**Princess Puzzle Game**](https://github.com/AieatAssam/bejewelled-clone) — ⭐ **7.5/10** · Claude Opus 4.5 · TypeScript, Three.js · 900 MFLOPS · _creator-reported_
- [**Tabularis Run**](https://github.com/TabularisDB/game) — ⭐ **7.5/10** · Claude Fable 5 · Canvas 2D, Web Audio API, JavaScript · 220 MFLOPS · _creator-reported_
- [**Tether**](https://github.com/Barabosik/tether-demo) — ⭐ **7.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 260 MFLOPS · _creator-reported_
- [**3D Minesweeper**](https://github.com/xoau79/3D-Minesweeper-Game-by-Fable-5) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 900 MFLOPS · _creator-reported_
- [**AstroHop**](https://github.com/nunoamorim99/AstroHop) — ⭐ **7.0/10** · Claude Fable 5 · JavaScript, Phaser 3, Vite · 650 MFLOPS · _creator-reported_
- [**Breakout**](https://github.com/shironagasu-ai/claude-fable5-games) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, Canvas 2D · 1.20 GFLOPS · _repository/topic trail_
- [**Breakout — ChatGPT Gauntlet Loop Experiment**](https://github.com/marcosmallet/breakout-gauntlet-loop) — ⭐ **7.0/10** · Claude Opus 5 · HTML, CSS, JavaScript, Canvas 2D · 220 MFLOPS · _directory-method_
- [**Mirror Breakout**](https://github.com/tbensonwest/claude-4-7-breakout-oneshot-demo) — ⭐ **7.0/10** · Claude Opus 4.7 · HTML, Canvas, JavaScript · 250 MFLOPS · _creator-reported_
- [**Pocket Soccer**](https://github.com/sbugrayy/Pocket-Soccer) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript, Canvas · 450 MFLOPS · _creator-reported_
- [**Snake**](https://github.com/shironagasu-ai/claude-fable5-games) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, Canvas 2D · 1.20 GFLOPS · _repository/topic trail_
- [**Snake**](https://github.com/IshaqAlathamneh/claude-snake-game) — ⭐ **7.0/10** · Claude Fable 5 · TypeScript, Browser · 180 MFLOPS · _creator-reported_
- [**Snaketris**](https://github.com/markstent/snaketris) — ⭐ **7.0/10** · Claude Fable 5 · JavaScript, Browser · 250 MFLOPS · _creator-reported_
- [**Sudoku**](https://github.com/antoniosubasic/sudoku) — ⭐ **7.0/10** · Claude Opus 4.5 · Svelte, Rust, WebAssembly · 150 MFLOPS · _creator-reported_
- [**Super Haaland Quest**](https://github.com/houssemeddinelassoued/haaland-quest-game) — ⭐ **7.0/10** · Claude Fable 5 · JavaScript, Canvas · 400 MFLOPS · _creator-reported_
- [**Tres Breakout**](https://github.com/alvarosabu/tres-breakout) — ⭐ **7.0/10** · Claude Fable 5 · JavaScript, TresJS, Three.js · 700 MFLOPS · _creator-reported_
- [**Flappy Bird Opus 4.7**](https://github.com/marcoakes/flappy-bird-opus-4-7) — ⭐ **6.5/10** · Claude Opus 4.7 · HTML5 Canvas, JavaScript · 220 MFLOPS · _creator-reported_
- [**VibePinball**](https://github.com/Randroids-Dojo/VibePinball) — ⭐ **6.5/10** · Claude Fable 5 · TypeScript, Canvas · 700 MFLOPS · _creator-reported_

[⬆️ Back to game library](#game-library)

## Three.js and WebGL

> 🧊 **53 verified games. Ranked by evidence-based quality score.**

- [**SUNBREAK — Downhill Club**](https://github.com/Imirushik/sunbreak-downhill-3D-game) — ⭐ **9.5/10** · GPT-6 Astra · Three.js, TypeScript, Vite, WebGL2, Web Audio API, Playwright · 7.50 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/Imirushik/sunbreak-downhill-3D-game/blob/main/media/SUNBREAK-showcase-poster.jpg)
- [**The Simpsons: Hit & Run — Browser Reconstruction**](https://github.com/Vheissu/hit-and-run-web) — ⭐ **9.5/10** · GPT-6 Astra · TypeScript, Three.js, WebGL, Vite, Blender, Browser · 8.80 GFLOPS · _confirmed_ · [📸 screenshot](https://github.com/Vheissu/hit-and-run-web/blob/main/docs/media/homer-and-marge.jpg) · [📸 screenshot](https://github.com/Vheissu/hit-and-run-web/blob/main/docs/media/downtown.jpg) · [📸 screenshot](https://github.com/Vheissu/hit-and-run-web/blob/main/docs/media/waterfront.jpg) · [📸 screenshot](https://github.com/Vheissu/hit-and-run-web/blob/main/docs/media/kwik-e-mart.jpg)
- [**Airlock Escape**](https://github.com/shironagasu-ai/claude-fable5-3D-games) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, cannon-es, JavaScript · 1.80 GFLOPS · _confirmed_
- [**Astra Dash**](https://github.com/octopus7/astracraft) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, Canvas 2D, WebGL, JavaScript · 1.20 GFLOPS · _confirmed_
- [**Aurora Sea**](https://github.com/shironagasu-ai/claude-fable5-3D-games) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, cannon-es, JavaScript · 1.80 GFLOPS · _confirmed_
- [**Cargo Crane**](https://github.com/shironagasu-ai/claude-fable5-3D-games) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, cannon-es, JavaScript · 1.80 GFLOPS · _confirmed_
- [**Cinder Yard**](https://github.com/octopus7/astracraft) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, Canvas 2D, WebGL, JavaScript · 1.20 GFLOPS · _confirmed_
- [**Fable City**](https://github.com/koviq4/fable-sandbox) — ⭐ **8.5/10** · Claude Fable 5.1 · Three.js, Vite, JavaScript · 900 MFLOPS · _confirmed_
- [**Flip Wall**](https://github.com/shironagasu-ai/claude-fable5-3D-games) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, cannon-es, JavaScript · 1.80 GFLOPS · _confirmed_
- [**Mosswing**](https://github.com/Ayi1337/gpt6-astra-one-shot-games) — ⭐ **8.5/10** · GPT-6 Astra · HTML, JavaScript, Canvas 2D, Three.js · 900 MFLOPS · _confirmed_
- [**Mosswing: Melon Lab**](https://github.com/Ayi1337/gpt6-astra-one-shot-games) — ⭐ **8.5/10** · GPT-6 Astra · HTML, JavaScript, Canvas 2D, Three.js · 900 MFLOPS · _confirmed_
- [**Star Runner**](https://github.com/shironagasu-ai/claude-fable5-3D-games) — ⭐ **8.5/10** · Claude Fable 5 · Three.js, cannon-es, JavaScript · 1.80 GFLOPS · _confirmed_
- [**The Mender's Quarter**](https://github.com/octopus7/astracraft) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, Canvas 2D, WebGL, JavaScript · 1.20 GFLOPS · _confirmed_
- [**The Rain Court**](https://github.com/octopus7/astracraft) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, Canvas 2D, WebGL, JavaScript · 1.20 GFLOPS · _confirmed_
- [**VOIDWRENCH**](https://github.com/octopus7/astracraft) — ⭐ **8.5/10** · GPT-6 Astra · Three.js, Canvas 2D, WebGL, JavaScript · 1.60 GFLOPS · _confirmed_
- [**Web-Slinger**](https://github.com/koviq4/fable-sandbox) — ⭐ **8.5/10** · Claude Fable 5.1 · Three.js, Vite, JavaScript · 900 MFLOPS · _confirmed_
- [**Celadon: The Long Ash**](https://github.com/winchxyz/celadon) — ⭐ **8.0/10** · Claude Opus 5 · JavaScript, WebGL, Canvas · 2.20 GFLOPS · _creator-reported_
- [**Fable Katamari**](https://github.com/aieo-product/fableDemoGame) — ⭐ **8.0/10** · Claude Fable 5 · JavaScript, Vite, Three.js · 2.60 GFLOPS · _creator-reported_
- [**FC 27**](https://github.com/Time-dev-alt/Fable-5.1-Games) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL, Canvas 2D · 1.20 GFLOPS · _creator-reported_
- [**Hydra Minecraft**](https://github.com/nickthelegend/hydra-minecraft) — ⭐ **8.0/10** · Claude Fable 5 · JavaScript, Three.js, WebSockets · 4.20 GFLOPS · _creator-reported_
- [**Joyride**](https://github.com/nitzangames/joyride) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, Web Audio API · 1.80 GFLOPS · _confirmed_
- [**Leonida**](https://github.com/AlfredBrowser/Leonida-GTA-6-Remake-Fable-5.1) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL · 900 MFLOPS · _confirmed_
- [**Leonida Stories 3D**](https://github.com/marvijo-code/leonida-stories-3d) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, Cannon-es, Web Audio API · 2.40 GFLOPS · _confirmed_
- [**Sakura — Petals of the Everblossom**](https://github.com/danmana/sakura-idle) — ⭐ **8.0/10** · Claude Opus 5 · Three.js, JavaScript, Web Audio API · 900 MFLOPS · _confirmed_
- [**Trashketball (Extra High 2)**](https://github.com/swathidbhat/gpt6-astra-extrahigh-codex-trashketball-2) — ⭐ **8.0/10** · GPT-6 Astra · React, TypeScript, Three.js, WebGL, Vite · 1.80 GFLOPS · _creator-reported_
- [**VALORANT Ascent**](https://github.com/Time-dev-alt/Fable-5.1-Games) — ⭐ **8.0/10** · Claude Fable 5.1 · Three.js, JavaScript, WebGL, Canvas 2D · 1.20 GFLOPS · _creator-reported_
- [**3D Game Vision Benchmark**](https://github.com/Yaxyobek0877/ai-model-benchmark-3d-game-vision) — ⭐ **7.5/10** · Claude Opus 5, Claude Fable 5 · HTML, JavaScript, Three.js · 1.80 GFLOPS · _creator-reported_
- [**Balance Astronaut**](https://github.com/asmoyou/toy2game) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, TypeScript, Vite, cannon-es · 1.80 GFLOPS · _repository/topic trail_
- [**Fable City GTA**](https://github.com/Octavious/Fable5_GTA) — ⭐ **7.5/10** · Claude Fable 5 · Three.js, Vite, TypeScript · 1.20 GFLOPS · _creator-reported_
- [**Fantasy League vs Field of Dreams**](https://github.com/mzaiger/FantasyLeagueVsFieldOfDreams) — ⭐ **7.5/10** · Claude Opus 5 · Three.js, JavaScript, HTML · 1.20 GFLOPS · _creator-reported_
- [**Frog Feast**](https://github.com/asmoyou/toy2game) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, TypeScript, Vite, cannon-es · 1.80 GFLOPS · _repository/topic trail_
- [**Jelly Baby**](https://github.com/scottstts/Jelly-Baby) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, WebGPU, JavaScript · 2.90 GFLOPS · _creator-reported_
- [**Korovany**](https://github.com/yunit00/game-corovans) — ⭐ **7.5/10** · Claude Fable 5 · HTML, JavaScript, Three.js · 2.20 GFLOPS · _creator-reported_
- [**Parking Escape**](https://github.com/asmoyou/toy2game) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, TypeScript, Vite, cannon-es · 1.80 GFLOPS · _repository/topic trail_
- [**Penguin Ice**](https://github.com/asmoyou/toy2game) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, TypeScript, Vite, cannon-es · 1.80 GFLOPS · _repository/topic trail_
- [**Rabbit Trap**](https://github.com/asmoyou/toy2game) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, TypeScript, Vite, cannon-es · 1.80 GFLOPS · _repository/topic trail_
- [**Trashketball (Extra High)**](https://github.com/swathidbhat/gpt6-astra-extrahigh-codex-trashketball) — ⭐ **7.5/10** · GPT-6 Astra · React, TypeScript, Three.js, WebGL, Vite · 1.70 GFLOPS · _creator-reported_
- [**Trashketball (Medium 2)**](https://github.com/swathidbhat/gpt6-astra-medium-codex-trashketball-2) — ⭐ **7.5/10** · GPT-6 Astra · React, TypeScript, Three.js, WebGL, Vite · 1.30 GFLOPS · _creator-reported_
- [**CHOP PRICER**](https://github.com/hurtz/choppricer) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, JavaScript, WebGL · 900 MFLOPS · _directory-method_
- [**Claude Bandicoot**](https://github.com/atiqur-rahman-pro/claude-bandicoot) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, Web Audio API · 900 MFLOPS · _directory-method_
- [**Claude of Tanks**](https://github.com/Kevin-Liu-01/Claude-of-Tanks) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, TypeScript, Vite · 1.20 GFLOPS · _directory-method_
- [**Deephaul**](https://github.com/TESTYEE-09/Deephaul-opus5-) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, TypeScript, Node.js, WebRTC · 900 MFLOPS · _repository/topic trail_
- [**Fable5 Minecraft**](https://github.com/Angais/Fable5-mc) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.50 GFLOPS · _repository/topic trail_
- [**Groot Theft Bakkie**](https://github.com/MichaelBernhardt/GTB6) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, TypeScript, Vite · 1.20 GFLOPS · _directory-method_
- [**Gura Gura Iseki**](https://github.com/simosimo1333-png/5p-games-opus5) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, Rapier, JavaScript, WebSocket · 1.80 GFLOPS · _repository/topic trail_
- [**Hitman 2016 Demo**](https://github.com/EricSpencer00/oneshot-hm2016) — ⭐ **7.0/10** · Claude Opus 4.5 · JavaScript, Three.js · 1.80 GFLOPS · _creator-reported_
- [**Memory**](https://github.com/shironagasu-ai/claude-fable5-games) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, Canvas 2D · 1.20 GFLOPS · _repository/topic trail_
- [**Minecraft Clone Fable5**](https://github.com/kengonakajima/mc_clone_fable5) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, WebGL · 1.20 GFLOPS · _repository/topic trail_
- [**Mirror Tap**](https://github.com/shironagasu-ai/claude-fable5-games) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, Canvas 2D · 1.20 GFLOPS · _repository/topic trail_
- [**NeonBeat**](https://github.com/OBdangshang07/Opus5_rhythm_game) — ⭐ **7.0/10** · Claude Opus 5 · Canvas 2D, WebGL2, Vue, TypeScript, Web Audio API · 900 MFLOPS · _repository/topic trail_
- [**Reflex Tap**](https://github.com/shironagasu-ai/claude-fable5-games) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, Canvas 2D · 1.20 GFLOPS · _repository/topic trail_
- [**Signal Drive**](https://github.com/shironagasu-ai/claude-fable5-games) — ⭐ **7.0/10** · Claude Fable 5 · Three.js, JavaScript, Canvas 2D · 1.20 GFLOPS · _repository/topic trail_
- [**Starfall**](https://github.com/e01-ai/starfall) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, TypeScript, Vite · 900 MFLOPS · _directory-method_

[⬆️ Back to game library](#game-library)

## Strategy, Simulation, and Sports

> ♟️ **15 verified games. Ranked by evidence-based quality score.**

- [**Farm Game**](https://github.com/BrilZliaN/farm-game) — ⭐ **8.0/10** · Claude Opus 5 · TypeScript, Three.js, Vite · 1.50 GFLOPS · _creator-reported_
- [**Tribal Gods**](https://github.com/br3nt/tribal-gods) — ⭐ **8.0/10** · Claude Fable 5 · TypeScript, Three.js · 3 GFLOPS · _creator-reported_
- [**Bubble Wrap Simulator**](https://github.com/finktheartist/bubble-wrap-simulator) — ⭐ **7.5/10** · GPT-6 Astra · Three.js, React, Rapier, Web Audio API · 1.30 GFLOPS · _creator-reported_
- [**Landlord: London 2026**](https://github.com/teddyjames1995/Claudopoly) — ⭐ **7.5/10** · Claude Opus 5 · JavaScript, Three.js · 1.10 GFLOPS · _creator-reported_
- [**Tiny Kingdom**](https://github.com/theniki/Tiny-Kingdom) — ⭐ **7.5/10** · Claude Opus 4.7 · JavaScript, Phaser 3, Vite · 650 MFLOPS · _creator-reported_
- [**Trading Card Game Fable5**](https://github.com/jbrwilkinson/Trading-Card-Game-Fable5) — ⭐ **7.5/10** · Claude Fable 5 · HTML, JavaScript · 220 MFLOPS · _creator-reported_ · [🧠 prompt](https://github.com/jbrwilkinson/Trading-Card-Game-Fable5/blob/main/docs/prompt.txt)
- [**Abstracts**](https://github.com/Alex-Caian/abstracts) — ⭐ **7.0/10** · Claude Fable 5 · JavaScript, Browser · 350 MFLOPS · _creator-reported_
- [**Deck Shenanigans**](https://github.com/burakTanBilgi/deck-shenanigans) — ⭐ **7.0/10** · Claude Opus 5 · TypeScript, Browser · 220 MFLOPS · _creator-reported_
- [**Gridiron GM**](https://github.com/VaultSparkStudios/gridiron-gm) — ⭐ **7.0/10** · Claude Opus 4.6 · JavaScript, Browser · 300 MFLOPS · _creator-reported_
- [**Iron Resolve**](https://github.com/rubenvoss07-hub/GauntletGame) — ⭐ **7.0/10** · Claude Opus 5 · JavaScript, HTML, CSS · 260 MFLOPS · _directory-method_
- [**Koroba 006**](https://github.com/mrgdata/koroba_py) — ⭐ **7.0/10** · Claude Opus 4.6 · Python, Flask, HTML · 250 MFLOPS · _creator-reported_
- [**Tiny Lives**](https://github.com/Manonsigilla/Sims-Fable-5) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript · 300 MFLOPS · _creator-reported_
- [**Verdium Storm**](https://github.com/jimskin03/Verdium-Storm) — ⭐ **7.0/10** · Claude Opus 5 · Three.js, TypeScript, Vite · 1.10 GFLOPS · _directory-method_
- [**Dune Browser Game**](https://github.com/cocodedk/Dune-Browser-Game) — ⭐ **6.5/10** · Claude Opus 5 · Three.js, React, TypeScript, Vite · 1.50 GFLOPS · _directory-method_
- [**Frosty Tactics**](https://github.com/Ninaji/Frosty-Tatics) — ⭐ **6.5/10** · Claude Fable 5 · HTML, JavaScript · 350 MFLOPS · _creator-reported_

[⬆️ Back to game library](#game-library)

## Other Browser Games

> 🎮 **73 verified games. Ranked by evidence-based quality score.**

- [**Blocky Obby Dash**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Bluff Circle**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Carnival Cannon Crush**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Circle Down**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Combo Grid**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Last Keep**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**LUMA — The Garden of Light**](https://github.com/TetsuakiBaba/2DAction.bench) — ⭐ **8.5/10** · GPT-6 Astra · HTML, Vanilla JavaScript, Canvas 2D, Web Audio API · 320 MFLOPS · _confirmed_ · [📸 screenshot](https://github.com/TetsuakiBaba/2DAction.bench/blob/main/GPT6-Astra/screenshot.png)
- [**Perfect Season Draft**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Purrfect Logic**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Red Alert 2 Browser Recreation**](https://github.com/xinbenlv/ra2-gpt-6-astra-2026-09-04) — ⭐ **8.5/10** · GPT-6 Astra · TypeScript, Vite, Canvas 2D, Web Audio API · 3.80 GFLOPS · _confirmed_
- [**Seaside Scoop**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Slide & Exit**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Star Gulp**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Starlight Peaks**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Sudden Death Strikers**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Vector Vacate**](https://github.com/pawanp3/renderwolf-fable5-chart-toppers) — ⭐ **8.5/10** · Claude Fable 5 · HTML, JavaScript, Canvas 2D · 220 MFLOPS · _confirmed_
- [**Last Beacon**](https://github.com/stackloomdev/last-beacon) — ⭐ **8.0/10** · GPT-6 Astra · Canvas 2D, JavaScript, Web Audio API · 260 MFLOPS · _confirmed_
- [**Mainspring**](https://github.com/Kelevera/mainspring) — ⭐ **8.0/10** · Claude Fable 5 · JavaScript, HTML, Canvas · 550 MFLOPS · _creator-reported_
- [**VoxelCraft**](https://github.com/UkawaJun/Fable5-VoxelCraftGame) — ⭐ **8.0/10** · Claude Fable 5 · Python, NumPy, Voxel rendering · 7 GFLOPS · _creator-reported_
- [**37 original Roblox party minigames**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Beat Streets — Web**](https://github.com/Lewiscowles1986/beatstreets-online) — ⭐ **7.5/10** · Claude Opus 5 · TypeScript, React, Vite, Canvas · 220 MFLOPS · _creator-reported_
- [**Bomberman Clone**](https://github.com/lifeofnicolas/bomberman-clone) — ⭐ **7.5/10** · Claude Fable 5.1 · HTML5 Canvas, JavaScript, Web Audio API · 300 MFLOPS · _creator-reported_
- [**Ember Run**](https://github.com/mbilalameen/games.ai) — ⭐ **7.5/10** · GPT-6 Astra · Phaser 3, JavaScript, HTML Canvas, Web Audio API · 450 MFLOPS · _creator-reported_
- [**Party Chaos — game 10**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 11**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 12**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 13**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 14**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 15**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 16**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 17**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 18**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 19**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 2**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 20**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 21**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 22**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 23**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 24**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 25**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 26**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 27**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 28**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 29**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 3**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 30**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 31**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 32**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 33**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 34**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 35**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 36**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 37**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 4**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 5**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 6**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 7**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 8**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Party Chaos — game 9**](https://github.com/lawrenceandrewlargo/roblox-extreme-minigames) — ⭐ **7.5/10** · Claude Fable 5.1 · Roblox, Luau · 220 MFLOPS · _creator-reported_
- [**Sonar Maze**](https://github.com/endurovojta173/Sonar_Maze_Profiq_Contest) — ⭐ **7.5/10** · Claude Opus 5 · HTML, JavaScript · 400 MFLOPS · _creator-reported_
- [**Age of Ops**](https://github.com/antebm/fabled-games) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript · 400 MFLOPS · _creator-reported_
- [**DUSTMAKER**](https://github.com/eriknomitch/dustmaker) — ⭐ **7.0/10** · Claude Opus 5 · HTML, JavaScript, Canvas · 220 MFLOPS · _directory-method_
- [**Gorillas**](https://github.com/aarongarciah/gorillas) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript · 220 MFLOPS · _creator-reported_
- [**Gran Mayhem**](https://github.com/antebm/fabled-games) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript · 400 MFLOPS · _creator-reported_
- [**SPEEDHELL**](https://github.com/jacobv25/speedhell) — ⭐ **7.0/10** · Claude Opus 5 · JavaScript, HTML Canvas · 220 MFLOPS · _directory-method_
- [**The Fable Forge**](https://github.com/vivmuk/fable-forge) — ⭐ **7.0/10** · Claude Fable 5 · HTML, JavaScript · 300 MFLOPS · _creator-reported_
- [**Vespertine**](https://github.com/sunkencity999/vespertine) — ⭐ **7.0/10** · Claude Fable 5 · HTML5 Canvas, JavaScript · 300 MFLOPS · _repository/topic trail_
- [**Afterglow**](https://github.com/ThakornToon/neon-runner) — ⭐ **6.5/10** · Claude Fable 5 · JavaScript, Browser · 450 MFLOPS · _creator-reported_
- [**Bird Tic-Toe**](https://github.com/vriveras/bird-tic-toe) — ⭐ **6.5/10** · Claude Opus 4.5 · HTML, JavaScript · 150 MFLOPS · _creator-reported_
- [**Claude's Game**](https://github.com/johnvsbabylon/claudes_game) — ⭐ **6.5/10** · Claude Opus 4.5 · HTML, JavaScript · 350 MFLOPS · _creator-reported_
- [**Moonberry Trail**](https://github.com/the-missionary-company/moonberry-trail) — ⭐ **6.5/10** · Claude Opus 5 · JavaScript, HTML, CSS · 300 MFLOPS · _directory-method_
- [**Raccoon Heist**](https://github.com/simonw/raccoon-heist) — ⭐ **6.5/10** · Claude Fable 5 · HTML, JavaScript · 350 MFLOPS · _creator-reported_
- [**The Unwritten Vale: Endless**](https://github.com/DAH746/fable-games) — ⭐ **6.5/10** · Claude Fable 5 · HTML, JavaScript · 350 MFLOPS · _creator-reported_

[⬆️ Back to game library](#game-library)

## Method

> 🔬 **Proof over promises. Repository evidence decides what gets counted.**

- Verify the canonical GitHub repository and numeric repository ID.
- Inspect the README, entry point, and gameplay source. Confirm input, rules or objectives, and game state.
- Place Godot, Unity, Unreal, Pygame, native-console, Minecraft, and other non-browser engine games in the dedicated non-browser section.
- Accept creator, repository, directory, or build-log evidence for Claude Opus, Claude Fable, or GPT-6 Astra. Label the evidence level.
- Do not count catalogs, skills, screenshots, visual-only scenes, empty repositories, unchanged forks, or prompt-only projects.
- Keep the source records in [`games.json`](games.json). Keep rejected candidates in [`research/candidates.json`](research/candidates.json).

## 📐 Rating and FLOPS notes

The **0–10 rating** uses source completeness, playable mechanics, scope, tests or deployment, and model-evidence strength. The **FLOPS value** is a low-confidence static estimate of FP32 work per second at 60 FPS. It is not measured device performance and it is not model-training compute.

## 🤝 Contributing

Open an issue or pull request with the game repository, model evidence, playable-source path, screenshot, and original prompt when available. New entries must pass the same verification rules.

## ⭐ Star this collection

If source-backed AI game history should stay searchable, [**star the repository**](https://github.com/agents-dev/find-games-last-week-made-with). The robots made the games. Humans still have to curate the receipts. 😏
